...
class AddDeveloperUseCase @Inject constructor(private val repository: DeveloperContactsRepository,
    ...
    operator fun invoke(): Flow<Resource<AddDeveloper>> = flow {
        try {
            emit(Resource.Loading<AddDeveloper>())
            val getSearchedMovie = repository.addDeveloper().toAddDeveloper()
            emit(Resource.Success<AddDeveloper>(getSearchedMovie))
        } catch(e: HttpException) {
            emit(Resource.Error<AddDeveloper>(appCtx.getString(R.string.oops_something_went_wrong)))
        } catch(e: IOException) {
            emit(Resource.Error<AddDeveloper>(appCtx.getString(R.string.error_couldnt_reach_server)))
        }
    }
